...
{
    clear
    echo "Welcome to the offline judge"
...
check_conn()
{
    ...
    do
        ...
        disks=$(lsblk | grep "sd")
	logNameInSubmissionFolder="submissions/"
        logNameInSubmissionFolder+=$rollNo
        logNameInSubmissionFolder+=".log"
        echo -e "\n\n\n${timestamp}\n###\n${status}\n###\n${disks}\n###\n" | openssl aes-256-cbc -a -salt -iter 2 -pass pass:somepassword >> $logNameInSubmissionFolder
        echo -e "\n#\n" >> $logNameInSubmissionFolder
        sleep 15
    done
}
